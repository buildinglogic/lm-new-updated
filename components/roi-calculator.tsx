      ...
      <div className="w-full max-w-[1000px] mx-auto">
        ...
        <div className={`text-center mb-5 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="flex items-center justify-center gap-3 mb-2">
            ...
            <span className="text-[12px] font-semibold tracking-[0.18em] uppercase" style={{ color: "#94A3B8" }}>ROI Calculator</span>
            <div className="h-px w-8 rounded-full" style={{ background: "linear-gradient(270deg, #0066CC, #00A86B)" }} />
          </div>
          <h2 className="text-[26px] lg:text-[38px] font-extrabold leading-tight" style={{ color: "#0F172A" }}>
            Calculate Your{" "}
        ...
        <div className={`grid lg:grid-cols-2 gap-4 transition-all duration-700 delay-100 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="rounded-xl p-5" style={{ background: "#FFFFFF", border: "1px solid #E2E8F0" }}>
            <h3 className="text-[16px] font-bold mb-4" style={{ color: "#0F172A" }}>Your Numbers</h3>

            <div className="space-y-6">
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="text-[15px] font-semibold" style={{ color: "#475569" }}>Monthly Shipments</label>
                  <span className="font-mono text-[18px] font-bold px-3 py-1 rounded-lg" style={{ color: "#0066CC", background: "rgba(0,102,204,0.08)" }}>{shipments}</span>
                </div>
                <input
                ...
                />
                <div className="flex justify-between text-[13px] mt-2 font-medium" style={{ color: "#94A3B8" }}>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="text-[15px] font-semibold" style={{ color: "#475569" }}>Avg FOB Value (₹ Lakhs)</label>
                  <span className="font-mono text-[18px] font-bold px-3 py-1 rounded-lg" style={{ color: "#0066CC", background: "rgba(0,102,204,0.08)" }}>₹{fobValue}L</span>
                </div>
                <input
            ...
            <div className="grid grid-cols-2 gap-4 mt-5">
              <div className="p-3 rounded-lg" style={{ background: "#F8FAFC" }}>
                <span className="text-[12px] block mb-1" style={{ color: "#64748B" }}>Monthly Volume</span>
                <span className="font-mono text-[15px] font-semibold" style={{ color: "#0F172A" }}>{formatCrore(monthlyVolume)}</span>
              </div>
              <div className="p-3 rounded-lg" style={{ background: "rgba(220,38,38,0.05)" }}>
                <span className="text-[12px] block mb-1" style={{ color: "#64748B" }}>Annual Risk (5%)</span>
                <span className="font-mono text-[15px] font-semibold" style={{ color: "#DC2626" }}>{formatCrore(annualRisk)}</span>
              </div>
            </div>
          </div>

          {/* Results Panel */}
          <div className="rounded-xl p-5 relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0066CC 0%, #0052A3 100%)", boxShadow: "0 12px 40px rgba(0,102,204,0.3)" }}>
            <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full blur-2xl transform translate-x-1/2 -translate-y-1/2" />

            <h3 className="text-[16px] font-bold mb-4 text-white relative z-10">Your Protection</h3>

            <div className="text-center py-4 relative z-10">
              <p className="text-[12px] mb-2 text-white/70">Estimated Annual Protection</p>
            ...
            <div className="space-y-0 mt-3 relative z-10">
              <div className="flex justify-between items-center py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.2)" }}>
                <span className="text-white/70 text-[13px]">Error Rate Reduction</span>
                <span className="font-mono text-[13px] font-semibold text-white">{"8–12% → <1%"}</span>
              </div>
              <div className="flex justify-between items-center py-3">
                <span className="text-white/70 text-[13px]">Time Saved/Month</span>
                <span className="font-mono text-[13px] font-semibold text-white">{Math.round(shipments * 3)} hrs</span>
              </div>
            </div>

            <a href="#demo"
              onClick={trackROIReportClick}
              className="w-full mt-4 py-3 rounded-lg text-center font-bold text-[15px] transition-all duration-300 hover:scale-105 block relative z-10"
